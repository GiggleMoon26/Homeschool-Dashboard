...
export default async function KidChecklistPage() {
  ...
  const { data: items } = await admin
    ...
    .select('code, title, plain_explanation, subject, is_ongoing, markoff_criteria, done')
            ...
            <div key={subject} className="mb-6">
              ...
              <div className="flex flex-col gap-1">
                {subjectItems.map((item) => (
                  <div key={item.code} className={`p-2 rounded ${item.done ? 'text-slate-500' : ''}`}>
                    <div className={`flex items-center gap-2 text-sm ${item.done ? 'line-through' : ''}`}>
                      <span>{item.done ? '✅' : '⬜'}</span>
                    ...
                    {item.plain_explanation && (
                      <div className="text-xs text-slate-600 italic ml-6 mt-0.5">{item.plain_explanation}</div>
                    )}
                    {item.markoff_criteria && (
                      <div className="text-xs text-slate-600 ml-6 mt-0.5">Mark off when: {item.markoff_criteria}</div>
                    )}
                  </div>
                ))}
              </div>
