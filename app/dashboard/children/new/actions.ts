  ...
  if (seeds.length > 0 && newChild) {
    const { error: checklistError } = await supabase.from('checklist_items').insert(
      seeds.map((s) => ({
        ...
        code: s.code,
        title: s.title,
        plain_explanation: s.plainExplanation || null,
        subject: s.subject,
    ...
    if (checklistError) {
      // Without this, a failed checklist seed (e.g. a missing column) would
      // leave a child with a genuinely empty checklist and zero indication
      // why — the same silent-failure trap we've hit before elsewhere.
      redirect('/dashboard?error=' + encodeURIComponent(
        `${name} was added, but seeding their checklist failed: ${checklistError.message}`
      ));
    }
  } else if (newChild) {
