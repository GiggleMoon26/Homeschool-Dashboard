...
import { addTask, updateTask, deleteTask, toggleTaskParent, resetRecurringTasks, bulkAddTasks } from './actions';
...
export default async function TaskManagerPage({
  ...
  const { error, edit: editingTaskId } = await searchParams;

  const supabase = await createClient();

  const { data: child } = await supabase
    .from('child_profiles')
    .select('id, name, avatar, family_id')
    .eq('id', childId)
    .single();
  ...
  const { data: tasks } = await supabase
    .from('tasks')
    .select('id, subject, description, code, markoff, resource, activity_type, days, is_recurring, specific_date, done, date_completed')
    .eq('child_id', childId)
  ...
  const updateTaskWithId = editingTask ? updateTask.bind(null, childId, editingTask.id) : null;

  const dateTasks = (tasks || []).filter((t) => t.specific_date);
  const flexTasks = (tasks || []).filter((t) => !t.specific_date && (!t.days || t.days.length === 0));
  const dayTasks = (tasks || []).filter((t) => !t.specific_date && t.days && t.days.length > 0);
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white p-6">
      <div className="max-w-2xl mx-auto">
        ...
        {editingTask && updateTaskWithId ? (
          <AddTaskForm
            ...
            initialValues={{
              ...
              markoff: editingTask.markoff,
              resource: editingTask.resource,
              activity_type: editingTask.activity_type,
              days: editingTask.days || [],
              is_recurring: editingTask.is_recurring,
              specific_date: editingTask.specific_date,
            }}
          />
        ) : (
          <AddTaskModal action={addTaskWithId} checklist={checklist || []} />
        )}
        ...
        <h2 className="text-lg font-semibold mt-8 mb-3">📅 Specific dates ({dateTasks.length})</h2>
        <div className="flex flex-col gap-2 mb-8">
          {dateTasks.length === 0 && <p className="text-slate-500 text-sm">None yet — e.g. an excursion or a one-off event.</p>}
          {[...dateTasks].sort((a, b) => (a.specific_date || '').localeCompare(b.specific_date || '')).map((t) => (
            <TaskRow key={t.id} task={t} childId={childId} onToggle={toggleTaskParent} onDelete={deleteTask} />
          ))}
        </div>

        <h2 className="text-lg font-semibold mb-3">Repeats on weekday(s) ({dayTasks.length})</h2>
        <div className="flex flex-col gap-2 mb-8">
        ...
        <h2 className="text-lg font-semibold mb-3">Not yet allocated ({flexTasks.length})</h2>
        <p className="text-xs text-slate-500 mb-3">These stay hidden from {child!.name} until you give them a day or date above.</p>
...
function TaskRow({
  ...
  task: { id: string; subject: string; description: string; code: string | null; days: string[]; is_recurring: boolean; specific_date: string | null; done: boolean; date_completed: string | null };
  childId: string;
    ...
    <div className={`flex items-start gap-3 p-3 rounded-lg border border-slate-700 bg-slate-900 ${task.done ? 'opacity-50' : ''}`}>
      ...
      <div className="flex-1">
        ...
        <div className="text-xs text-slate-500 mt-1 flex gap-2 flex-wrap items-center">
          ...
          {task.specific_date && <span className="bg-cyan-900 text-cyan-200 px-2 py-0.5 rounded">📅 {task.specific_date}</span>}
      ...
      <Link href={`?edit=${task.id}`} className="text-xs text-cyan-400 underline">Edit</Link>
